...
async function bookingActive(request, response) {
    ...
    if(paid) {
        const data = paid.filter((value) => {
            ...
            logger.log("info", new Date(`${currentDate.getFullYear()}-${currentDate.getMonth() + 1}-${currentDate.getDay()}`).getTime()/1000)
            ...
            return (new Date(`${currentDate.getFullYear()}-${currentDate.getMonth() + 1}-${currentDate.getDay()}`).getTime()/1000) < (new Date(value.booking_date).getTime()/1000) && new moment(new Date().getTime()).tz('Asia/Jakarta').unix() < (addHourToDate(value.booking_date, parseInt(value.booking_time.split(':')[0])) / 1000);
        })
